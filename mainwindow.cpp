    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->CIN_4->setValidator(new QIntValidator(0, 99999999, this));
    ui->modify->hide();
    ui->affichage->hide();
...
void MainWindow::on_comboBox_currentIndexChanged(int index) // Tri
{
    if(index == 1)
...
void MainWindow::on_pushButton_3_clicked() // Modification
{
    ui->modify->show();
    QModelIndex index = ui->tab_employe->currentIndex();
    ...
    ui->CIN_2->setText(email);
    ui->CIN_6->setText(adresse);
    if(role == "Manager")
        ui->role_2->setCurrentIndex(0);
    else
        ui->role_2->setCurrentIndex(1);
    ui->date_naiss_2->setDate(QDate::fromString(date_naiss, "dd.MM.yyyy"));
}
...
void MainWindow::on_pushButton_9_clicked() // Click on Confirmer
{
    int CIN = ui->CIN_4->text().toInt();
    QString Nom = ui->CIN_5->text();
    QString Prenom = ui->CIN_3->text();
    QString email = ui->CIN_2->text();
    int Sexe_answer = ui->Sexe_2->currentIndex();
    QString Sexe;
    QString adresse = ui->CIN_6->text();
    QString date_naiss = ui->date_naiss_2->date().toString("dd.MM.yyyy");
    int Role_answer = ui->role_2->currentIndex();
    QString role;
    ...
    if(CIN < 9999999 || Nom == "" || Prenom == "" || email == ""){
        ui->CIN_4->setStyleSheet("border: 1px solid red;");
        if(CIN > 9999999)
            ui->CIN_4->setStyleSheet("");
        if(Nom == "")
            ui->CIN_5->setStyleSheet("border: 1px solid red;");
        else
            ui->CIN_5->setStyleSheet("");
        if(Prenom == "")
            ui->CIN_3->setStyleSheet("border: 1px solid red;");
        else
            ui->CIN_3->setStyleSheet("");
        if(email == "")
            ui->CIN_2->setStyleSheet("border: 1px solid red;");
        else
            ui->CIN_2->setStyleSheet("");
    }else{
        QMessageBox msgBox;
...
void MainWindow::on_pushButton_6_clicked() //clicking on Reset
{
    ui->CIN->setText("");
    ui->Nom->setText("");
    ui->Prenom->setText("");
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    ui->cin_text->setText(cin);
    ui->nom_text->setText(nom);
    ui->prenom_text->setText(prenom);
    ui->sexe_text->setText(sexe);
    ui->email_text->setText(email);
    ui->adresse_text->setText(adresse);
    ui->role_text->setText(role);
    ui->date_naiss_text->setText(date_naiss);
}
...
void MainWindow::on_pushButton_12_clicked() // PDF Button
{
    QModelIndex index = ui->tab_employe->currentIndex();
    QString cin = index.data(Qt::DisplayRole).toString();
    QString nom = ui->tab_employe->model()->index(index.row(), 1).data(Qt::DisplayRole).toString();
...
void MainWindow::on_CIN_recherche_textChanged(const QString &arg1) //Recherche avancé
{
